 ...
 * Master output meter.
 *
 * Small, but it answers the worst question a music app can leave you with —
 * "is it playing and I can't hear it, or is it not playing?" — without any
 * debugging. If these bars move and you hear nothing, the problem is the
 * device volume or a silent switch, not the app.
 */
function OutputMeter() {
  ...
  const bars = 5;
  const lit = Math.round(Math.min(1, level * 2.6) * bars);
  return (
    <div
      className="flex items-end gap-[2px]"
      title={level > 0.001 ? 'Audio is coming out' : 'No output — press play'}
      aria-hidden
    >
      {Array.from({ length: bars }, (_, i) => (
        <span
          ...
          style={{
            ...
            height: 5 + i * 3,
            background:
              i < lit ? (i >= bars - 1 ? 'rgb(var(--crit))' : 'rgb(var(--mint))') : 'rgb(var(--rule-2))',
          }}
        />
      ))}
    </div>
...
export default function Transport() {
  ...
  return (
    <div className="flex flex-wrap items-center gap-2 border-t-[1.5px] border-ink bg-panel2 px-3 py-2.5">
      ...
      {/* Tempo stays in simple mode: slowing a hard passage down is the single
          most useful thing here, and it is why most people open the app. */}
      <div className="flex items-center gap-2">
        <span className="lbl">Tempo</span>
        <input
          type="range"
          className="rng w-24 sm:w-28"
          min={28}
          max={220}
          value={transport.bpm}
        ...
        />
        <Num className="w-[62px] text-[12px]">
      ...
      <div className="ml-auto flex items-center gap-3">
        ...
        <input
          type="range"
          className="rng w-32 hidden sm:block"
          min={0}
